...
$size=$_GET['size'];
function getjscode($size,$location){
  $curl = curl_init();
  curl_setopt($curl, CURLOPT_URL, "https://widgets.waqi.info/jswgt/?size=$size&city=$location&lang=cn");
  curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($curl, CURLOPT_CONNECTTIMEOUT, 30);
  $str = curl_exec($curl);
  curl_close($curl);
...
$jscode=getjscode($size,$currentCityEName);
echo $jscode;
?>
